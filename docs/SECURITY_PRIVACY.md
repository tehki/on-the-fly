...
| Model weights verified before use | `infrastructure/model_store.py` | A model declaring no digests refuses to load; a file whose SHA-256 does not match its pin is refused and left in place for inspection; `allow_download` defaults to false. **Checked on every load**, so a file altered since the last run is caught |
| Translation archives verified when converted, not on every load | `infrastructure/translation/artifacts.py` | The CTranslate2 route pins and checks the publisher's `.zip`, then converts it and loads the conversion. There is no digest for the conversion — it is this project's own output and no publisher publishes one — so what is guaranteed is the weights it was **built from**, plus that an unfinished conversion is never served as a finished one. A converted model altered in the cache afterwards is not detected, where the ONNX route would detect it. Recorded because the two engines differ and the default is the weaker one |
